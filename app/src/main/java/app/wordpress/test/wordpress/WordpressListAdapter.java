...
public class WordpressListAdapter extends ArrayAdapter<PostItem> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null || convertView.getTag().equals(TAG_TOP)) {
			...
			holder.reportedRastView = (TextView) convertView.findViewById(R.id.rast);
        ...
        //set rest
		if (newsItem.getLatitude() != 0) {
			newsItem.setRast(CalculationDistanceByCoord(newsItem.getUserLatitude(),newsItem.getUserLongitude(),newsItem.getLatitude(),newsItem.getLongitude()));
			Float temp = (float)newsItem.getRast()/1000;
			Float temp1= (float)newsItem.getRast();
			holder.reportedRastView.setText("До цели " + temp.toString()+" километров ("+temp1.toString()+" метров)");
		}
