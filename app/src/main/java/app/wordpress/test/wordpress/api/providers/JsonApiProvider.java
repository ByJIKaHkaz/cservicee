...
import static app.wordpress.test.util.Helper.Latitude;
import static app.wordpress.test.util.Helper.Longitude;
...
public class JsonApiProvider implements WordpressProvider {
    ...
    private static final String PARAMS = "date_format=U&exclude=comments,categories";
    ...
    public static PostItem itemFromJsonObject(JSONObject post) throws JSONException {
        ...
        if (post.has("latitude")) {
            Object custom_fields = post.get("custom_fields");
            if (custom_fields instanceof JSONArray
                    && ((JSONArray) custom_fields).length() > 0) {
                custom_fields = ((JSONArray) custom_fields).getJSONObject(0);
            }

            if (custom_fields instanceof JSONObject
                    && ((JSONObject) custom_fields).has("latitude")) {
                item.setLongitude(((JSONObject) custom_fields)
                        .getDouble("latitude"));
            }
        }

        if (post.has("custom_fields")) {
            Object custom_fields = post.get("custom_fields");
            if (custom_fields instanceof JSONArray
                    && ((JSONArray) custom_fields).length() > 0) {
                custom_fields = ((JSONArray) custom_fields).getJSONObject(0);
            ...
            if (custom_fields instanceof JSONObject
                    && ((JSONObject) custom_fields).has("latitude")) {
                JSONArray latitude = ((JSONObject) custom_fields).getJSONArray("latitude");
                Object latit = latitude.get(0);
                item.setLatitude(new Double(latit.toString()));
            }
            if (custom_fields instanceof JSONObject
                    && ((JSONObject) custom_fields).has("longitude")) {
                JSONArray longitude = ((JSONObject) custom_fields).getJSONArray("longitude");
                Object longi = longitude.get(0);
                item.setLongitude(new Double(longi.toString()));
            }
        }
